package br.com.alura.forumhub.service.validation.resposta.cadastrar;

import br.com.alura.forumhub.dto.resposta.DadosCadastroResposta;
import br.com.alura.forumhub.exception.ValidacaoException;
import br.com.alura.forumhub.model.StatusTopico;
import br.com.alura.forumhub.model.Topico;
import br.com.alura.forumhub.repository.TopicoRepository;
import org.springframework.stereotype.Component;

@Component
public class ValidadorTopicoSolucionado implements ValidationCadastrarResposta {

    private final TopicoRepository repository;

    public ValidadorTopicoSolucionado(TopicoRepository repository) {
        this.repository = repository;
    }

    @Override
    public void validar(DadosCadastroResposta dados) {

        Topico topico = repository.findById(dados.idTopico())
                .orElseThrow(() -> new ValidacaoException("Tópico não encontrado"));

        if (topico.getStatus() == StatusTopico.SOLUCIONADO) {
            throw new ValidacaoException("Não é permitido responder um tópico solucionado");
        }
    }
}
